...
test("the admin list serves a bounded page instead of the whole catalogue", async ({ request }) => {
  const auth = await logIn(request);
  await createProduct({ name: "Альфа", slug: "alfa" });
  await createProduct({ name: "Бета", slug: "beta" });
  await createProduct({ name: "Гамма", slug: "gamma" });

  const response = await request.get("/api/admin/products?limit=2", {
...
test("the admin list refuses a page size above the ceiling", async ({ request }) => {
  const auth = await logIn(request);

  const response = await request.get("/api/admin/products?limit=5000", {
    headers: { cookie: auth.cookie },
  });
...
test("the admin list checks the session before it reads the page parameters", async ({
  request,
}) => {
  const response = await request.get("/api/admin/products?limit=5000");
