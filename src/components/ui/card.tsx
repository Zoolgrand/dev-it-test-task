...
function Card({
  ...
  return (
    <div
      ...
      data-size={size}
      className={cn(
        "group/card flex flex-col gap-(--card-spacing) overflow-hidden rounded-xl border border-outline-variant/40 bg-card py-(--card-spacing) text-body-md text-card-foreground shadow-sm [--card-spacing:--spacing(4)] has-data-[slot=card-footer]:pb-0 has-[>img:first-child]:pt-0 data-[size=sm]:[--card-spacing:--spacing(3)] data-[size=sm]:has-data-[slot=card-footer]:pb-0 *:[img:first-child]:rounded-t-xl *:[img:last-child]:rounded-b-xl",
        className,
      )}
      {...props}
    />
...
function CardTitle({ className, ...props }: React.ComponentProps<"div">): React.JSX.Element {
  return (
    <div
      ...
      className={cn(
        "font-heading text-headline-sm text-on-surface group-data-[size=sm]/card:text-body-lg",
        className,
      )}
      {...props}
    />
...
function CardFooter({ className, ...props }: React.ComponentProps<"div">): React.JSX.Element {
  return (
    <div
      ...
      className={cn(
        "flex items-center rounded-b-xl border-t border-outline-variant/40 bg-surface-container-low/60 p-(--card-spacing)",
        className,
      )}
      {...props}
    />
