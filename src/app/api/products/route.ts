...
export async function GET(request: NextRequest): Promise<Response> {
  ...
  const page = await listPublishedProducts({ take: limit, skip: offset });
  const body: PublicProductListResponse = {
    products: page.items,
    total: page.total,
    limit,
    offset,
