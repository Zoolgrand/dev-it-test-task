...
describe("writing product content", () => {
  ...
  it("names the field that failed, so the caller can show the error beside the input", async () => {
    ...
    if (outcome.status !== "invalid") throw new Error("expected a validation failure");
    expect(outcome.fieldErrors.description).toBeTruthy();
  });

  it("refuses a body of the wrong shape without reaching the database", async () => {
    const outcome = await updateProductContent("missing", "not an object");
